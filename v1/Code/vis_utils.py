...
@keras_export('keras.utils.model_act_to_dot')
def model_act_to_dot(model,
                 show_shapes=False,
                 show_dtype=False,
  ...
  Args:
    ...
    image_layers: List of layers that have visualization images (must be same 
        length as parameter layer images).
    layer_images: Visualization image filenames that associate to the layers 
        in the image_layers list (they must be same length).
  ...
  Raises:
    ValueError: if `model_act_to_dot` is called before the model is built.
  ...
  if not check_pydot():
    message = (
        ...
        'for plot_model_act/model_act_to_dot to work.')
  ...
  #print(image_layers)
  if len(image_layers) != len(layer_images):
    raise ValueError(
       'layer_images length = '+str(len(layer_images))+'must be of same '+
       'as image_layers, length = '+str(len(image_layers)))
  ...
  for i, layer in enumerate(layers):
    ...
    if isinstance(layer, wrappers.Wrapper):
      if expand_nested and isinstance(layer.layer,
        ...
        submodel_wrapper = model_act_to_dot(
            layer.layer,
            show_shapes,
            show_dtype,
            show_layer_names,
    ...
    if expand_nested and isinstance(layer, functional.Functional):
      submodel_not_wrapper = model_act_to_dot(
          layer,
          show_shapes,
          show_dtype,
          show_layer_names,
    ...
    if not expand_nested or not isinstance(
      ...
      #print("Label "+layer_name)
      if layer_name in image_layers:
        #print(layer_images[image_layers.index(layer_name)])
        node = pydot.Node(layer_id, shape="box", label="", image = layer_images[image_layers.index(layer_name)], imagescale='True')
        #node = pydot.Node(layer_id, label="", image = "cam.png")
      else:
        #print(label)
...
@keras_export('keras.utils.plot_model_act')
def plot_model_act(model,
               to_file='model.png',
               show_shapes=True,
               show_dtype=True,
               show_layer_names=True,
  ...
  tf.keras.utils.plot_model_act(model, to_file=dot_img_file, show_shapes=True)
  ```
  ...
  Raises:
    ValueError: if `plot_model_act` is called before the model is built.
